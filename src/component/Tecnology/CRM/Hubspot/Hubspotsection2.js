import React from "react";

const Hubspotsection2 = () => {
  return (
    <div>

<div className="flex justify-center py-10">
        <div className="w-[90%] flex flex-col items-center">
          <span className="lg:text-[28px] text-[20px] text-center font-[600] ">
            The Advantages of Working with Magbyte for Development
          </span>
          <div className=" pt-10 flex flex-col gap-5">
            <div className="flex gap-5 lg:flex-row flex-col">
              <div className="lg:py-3 lg:px-5 px-3 py-2 lg:w-[400px] border  border-blue-500 hover:scale-105 rounded-md flex flex-col gap-3">
                <svg
                  width="56"
                  height="56"
                  viewBox="0 0 56 56"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <path
                    d="M37.3333 0H18.6667C8.35735 0 0 8.35735 0 18.6667V37.3333C0 47.6426 8.35735 56 18.6667 56H37.3333C47.6426 56 56 47.6426 56 37.3333V18.6667C56 8.35735 47.6426 0 37.3333 0Z"
                    fill="#4C49E1"
                  />
                  <path
                    d="M25.3261 24.1534C25.168 23.9555 24.8514 23.916 24.6535 24.0741L22.0815 26.2508C21.9628 26.3299 21.9233 26.4881 21.9233 26.6068C21.9233 26.765 22.0025 26.884 22.0815 26.9629L24.6535 29.1395C24.7326 29.2187 24.8514 29.2582 24.9701 29.2582C25.1283 29.2582 25.2472 29.1789 25.3261 29.1C25.4843 28.9021 25.4843 28.5855 25.2469 28.4274L23.1102 26.6466L25.2469 24.8659C25.4846 24.6677 25.4846 24.3513 25.3261 24.1534Z"
                    fill="white"
                    stroke="white"
                    stroke-width="0.322114"
                  />
                  <path
                    d="M33.9184 26.3738L31.3464 24.1972C31.1485 24.039 30.8319 24.0785 30.6737 24.2765C30.5155 24.4744 30.5155 24.791 30.753 24.9491L32.8896 26.7299L30.753 28.5106C30.555 28.6688 30.5155 28.9854 30.6737 29.1833C30.753 29.3021 30.9111 29.3415 31.0298 29.3415C31.1485 29.3415 31.2672 29.3021 31.3465 29.2228L33.9184 27.0462C34.0372 26.967 34.0766 26.8088 34.0766 26.6901C34.0768 26.5717 34.0372 26.4924 33.9184 26.3738Z"
                    fill="white"
                    stroke="white"
                    stroke-width="0.322114"
                  />
                  <path
                    d="M29.3304 21.9005C29.093 21.8213 28.8159 21.9797 28.737 22.2171L26.0065 30.9627C25.9272 31.2001 26.0856 31.4772 26.3231 31.5562C26.3625 31.5562 26.4022 31.5957 26.4811 31.5957C26.6791 31.5957 26.877 31.477 26.9559 31.2395L29.6467 22.5335C29.7259 22.2563 29.6073 21.9798 29.3304 21.9005Z"
                    fill="white"
                    stroke="white"
                    stroke-width="0.322114"
                  />
                  <path
                    d="M35.8948 9H20.1057C18.879 9 17.8105 10.0288 17.8105 11.2952V43.7055C17.8105 44.9322 18.8394 46.0007 20.1057 46.0007H35.8948C37.1213 46.0007 38.1898 44.9719 38.1898 43.7055L38.1896 11.2952C38.1896 10.0685 37.1214 9 35.8948 9ZM36.9237 43.7055C36.9237 44.2989 36.4884 44.7344 35.8949 44.7344H20.106C19.5125 44.7344 19.0771 44.299 19.0771 43.7055V40.1835H36.9634L36.9631 43.7055H36.9237ZM36.9237 38.9565H19.0375V13.5903H36.9237V38.9565ZM36.9237 12.3239H19.0375V11.2951C19.0375 10.7016 19.4727 10.2663 20.0662 10.2663H25.7248C25.7643 10.5037 25.9621 10.6621 26.1996 10.6621H29.6819C29.9193 10.6621 30.1172 10.4642 30.1566 10.2663H35.8153C36.4087 10.2663 36.8441 10.7016 36.8441 11.2951L36.8445 12.3239H36.9237Z"
                    fill="white"
                    stroke="white"
                    stroke-width="0.322114"
                  />
                  <path
                    d="M29.0488 42.4832C29.0488 43.0735 28.5704 43.5518 27.9803 43.5518C27.3905 43.5518 26.9121 43.0735 26.9121 42.4832C26.9121 41.8934 27.3905 41.415 27.9803 41.415C28.5705 41.415 29.0488 41.8934 29.0488 42.4832Z"
                    fill="white"
                    stroke="white"
                    stroke-width="0.322114"
                  />
                </svg>

                <span className="lg:text-[20px] text-[16px] font-[600]  ">
                  Affordable Pricing
                </span>
                <p className="lg:text-[16px]  text-[12px]">
                  We offer competitive and affordable pricing to ensure you
                  receive top-quality HubSpot solutions without exceeding your
                  budget. Our goal is to deliver exceptional value, providing
                  the best services at a cost-effective rate to help your
                  business grow efficiently.
                </p>
              </div>

              <div className="lg:py-3 lg:px-5 px-3 py-2 lg:w-[400px] border  border-blue-500 hover:scale-105 rounded-md flex flex-col gap-3">
                <svg
                  width="56"
                  height="56"
                  viewBox="0 0 56 56"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <path
                    d="M37.3333 0H18.6667C8.35735 0 0 8.35735 0 18.6667V37.3333C0 47.6426 8.35735 56 18.6667 56H37.3333C47.6426 56 56 47.6426 56 37.3333V18.6667C56 8.35735 47.6426 0 37.3333 0Z"
                    fill="#4C49E1"
                  />
                  <path
                    d="M25.3261 24.1534C25.168 23.9555 24.8514 23.916 24.6535 24.0741L22.0815 26.2508C21.9628 26.3299 21.9233 26.4881 21.9233 26.6068C21.9233 26.765 22.0025 26.884 22.0815 26.9629L24.6535 29.1395C24.7326 29.2187 24.8514 29.2582 24.9701 29.2582C25.1283 29.2582 25.2472 29.1789 25.3261 29.1C25.4843 28.9021 25.4843 28.5855 25.2469 28.4274L23.1102 26.6466L25.2469 24.8659C25.4846 24.6677 25.4846 24.3513 25.3261 24.1534Z"
                    fill="white"
                    stroke="white"
                    stroke-width="0.322114"
                  />
                  <path
                    d="M33.9184 26.3738L31.3464 24.1972C31.1485 24.039 30.8319 24.0785 30.6737 24.2765C30.5155 24.4744 30.5155 24.791 30.753 24.9491L32.8896 26.7299L30.753 28.5106C30.555 28.6688 30.5155 28.9854 30.6737 29.1833C30.753 29.3021 30.9111 29.3415 31.0298 29.3415C31.1485 29.3415 31.2672 29.3021 31.3465 29.2228L33.9184 27.0462C34.0372 26.967 34.0766 26.8088 34.0766 26.6901C34.0768 26.5717 34.0372 26.4924 33.9184 26.3738Z"
                    fill="white"
                    stroke="white"
                    stroke-width="0.322114"
                  />
                  <path
                    d="M29.3304 21.9005C29.093 21.8213 28.8159 21.9797 28.737 22.2171L26.0065 30.9627C25.9272 31.2001 26.0856 31.4772 26.3231 31.5562C26.3625 31.5562 26.4022 31.5957 26.4811 31.5957C26.6791 31.5957 26.877 31.477 26.9559 31.2395L29.6467 22.5335C29.7259 22.2563 29.6073 21.9798 29.3304 21.9005Z"
                    fill="white"
                    stroke="white"
                    stroke-width="0.322114"
                  />
                  <path
                    d="M35.8948 9H20.1057C18.879 9 17.8105 10.0288 17.8105 11.2952V43.7055C17.8105 44.9322 18.8394 46.0007 20.1057 46.0007H35.8948C37.1213 46.0007 38.1898 44.9719 38.1898 43.7055L38.1896 11.2952C38.1896 10.0685 37.1214 9 35.8948 9ZM36.9237 43.7055C36.9237 44.2989 36.4884 44.7344 35.8949 44.7344H20.106C19.5125 44.7344 19.0771 44.299 19.0771 43.7055V40.1835H36.9634L36.9631 43.7055H36.9237ZM36.9237 38.9565H19.0375V13.5903H36.9237V38.9565ZM36.9237 12.3239H19.0375V11.2951C19.0375 10.7016 19.4727 10.2663 20.0662 10.2663H25.7248C25.7643 10.5037 25.9621 10.6621 26.1996 10.6621H29.6819C29.9193 10.6621 30.1172 10.4642 30.1566 10.2663H35.8153C36.4087 10.2663 36.8441 10.7016 36.8441 11.2951L36.8445 12.3239H36.9237Z"
                    fill="white"
                    stroke="white"
                    stroke-width="0.322114"
                  />
                  <path
                    d="M29.0488 42.4832C29.0488 43.0735 28.5704 43.5518 27.9803 43.5518C27.3905 43.5518 26.9121 43.0735 26.9121 42.4832C26.9121 41.8934 27.3905 41.415 27.9803 41.415C28.5705 41.415 29.0488 41.8934 29.0488 42.4832Z"
                    fill="white"
                    stroke="white"
                    stroke-width="0.322114"
                  />
                </svg>

                <span className="lg:text-[20px] text-[16px] font-[600]  ">
                  Excellence in Quality
                </span>
                <p className="lg:text-[16px]  text-[12px]">
                  We are committed to delivering the highest quality solutions,
                  ensuring that every service we provide meets the highest
                  standards. Our focus on excellence guarantees that you receive
                  reliable, effective, and impactful results tailored to your
                  business needs.
                </p>
              </div>

              <div className="lg:py-3 lg:px-5 px-3 py-2 lg:w-[400px] border  border-blue-500 hover:scale-105 rounded-md flex flex-col gap-3">
                <svg
                  width="56"
                  height="56"
                  viewBox="0 0 56 56"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <path
                    d="M37.3333 0H18.6667C8.35735 0 0 8.35735 0 18.6667V37.3333C0 47.6426 8.35735 56 18.6667 56H37.3333C47.6426 56 56 47.6426 56 37.3333V18.6667C56 8.35735 47.6426 0 37.3333 0Z"
                    fill="#4C49E1"
                  />
                  <path
                    d="M25.3261 24.1534C25.168 23.9555 24.8514 23.916 24.6535 24.0741L22.0815 26.2508C21.9628 26.3299 21.9233 26.4881 21.9233 26.6068C21.9233 26.765 22.0025 26.884 22.0815 26.9629L24.6535 29.1395C24.7326 29.2187 24.8514 29.2582 24.9701 29.2582C25.1283 29.2582 25.2472 29.1789 25.3261 29.1C25.4843 28.9021 25.4843 28.5855 25.2469 28.4274L23.1102 26.6466L25.2469 24.8659C25.4846 24.6677 25.4846 24.3513 25.3261 24.1534Z"
                    fill="white"
                    stroke="white"
                    stroke-width="0.322114"
                  />
                  <path
                    d="M33.9184 26.3738L31.3464 24.1972C31.1485 24.039 30.8319 24.0785 30.6737 24.2765C30.5155 24.4744 30.5155 24.791 30.753 24.9491L32.8896 26.7299L30.753 28.5106C30.555 28.6688 30.5155 28.9854 30.6737 29.1833C30.753 29.3021 30.9111 29.3415 31.0298 29.3415C31.1485 29.3415 31.2672 29.3021 31.3465 29.2228L33.9184 27.0462C34.0372 26.967 34.0766 26.8088 34.0766 26.6901C34.0768 26.5717 34.0372 26.4924 33.9184 26.3738Z"
                    fill="white"
                    stroke="white"
                    stroke-width="0.322114"
                  />
                  <path
                    d="M29.3304 21.9005C29.093 21.8213 28.8159 21.9797 28.737 22.2171L26.0065 30.9627C25.9272 31.2001 26.0856 31.4772 26.3231 31.5562C26.3625 31.5562 26.4022 31.5957 26.4811 31.5957C26.6791 31.5957 26.877 31.477 26.9559 31.2395L29.6467 22.5335C29.7259 22.2563 29.6073 21.9798 29.3304 21.9005Z"
                    fill="white"
                    stroke="white"
                    stroke-width="0.322114"
                  />
                  <path
                    d="M35.8948 9H20.1057C18.879 9 17.8105 10.0288 17.8105 11.2952V43.7055C17.8105 44.9322 18.8394 46.0007 20.1057 46.0007H35.8948C37.1213 46.0007 38.1898 44.9719 38.1898 43.7055L38.1896 11.2952C38.1896 10.0685 37.1214 9 35.8948 9ZM36.9237 43.7055C36.9237 44.2989 36.4884 44.7344 35.8949 44.7344H20.106C19.5125 44.7344 19.0771 44.299 19.0771 43.7055V40.1835H36.9634L36.9631 43.7055H36.9237ZM36.9237 38.9565H19.0375V13.5903H36.9237V38.9565ZM36.9237 12.3239H19.0375V11.2951C19.0375 10.7016 19.4727 10.2663 20.0662 10.2663H25.7248C25.7643 10.5037 25.9621 10.6621 26.1996 10.6621H29.6819C29.9193 10.6621 30.1172 10.4642 30.1566 10.2663H35.8153C36.4087 10.2663 36.8441 10.7016 36.8441 11.2951L36.8445 12.3239H36.9237Z"
                    fill="white"
                    stroke="white"
                    stroke-width="0.322114"
                  />
                  <path
                    d="M29.0488 42.4832C29.0488 43.0735 28.5704 43.5518 27.9803 43.5518C27.3905 43.5518 26.9121 43.0735 26.9121 42.4832C26.9121 41.8934 27.3905 41.415 27.9803 41.415C28.5705 41.415 29.0488 41.8934 29.0488 42.4832Z"
                    fill="white"
                    stroke="white"
                    stroke-width="0.322114"
                  />
                </svg>

                <span className="lg:text-[20px] text-[16px] font-[600]  ">
                  Adaptability
                </span>
                <p className="lg:text-[16px]  text-[12px]">
                  We prioritize flexibility in our services, offering tailored
                  solutions that can easily adapt to your unique business
                  requirements. Whether it's customizing HubSpot tools or
                  adjusting strategies, we ensure that our approach aligns with
                  your evolving needs.
                </p>
              </div>
            </div>
            <div className="flex gap-5 lg:flex-row flex-col">
              <div className="lg:py-3 lg:px-5 px-3 py-2 lg:w-[400px] border  border-blue-500 hover:scale-105 rounded-md flex flex-col gap-3">
                <svg
                  width="56"
                  height="56"
                  viewBox="0 0 56 56"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <path
                    d="M37.3333 0H18.6667C8.35735 0 0 8.35735 0 18.6667V37.3333C0 47.6426 8.35735 56 18.6667 56H37.3333C47.6426 56 56 47.6426 56 37.3333V18.6667C56 8.35735 47.6426 0 37.3333 0Z"
                    fill="#4C49E1"
                  />
                  <path
                    d="M25.3261 24.1534C25.168 23.9555 24.8514 23.916 24.6535 24.0741L22.0815 26.2508C21.9628 26.3299 21.9233 26.4881 21.9233 26.6068C21.9233 26.765 22.0025 26.884 22.0815 26.9629L24.6535 29.1395C24.7326 29.2187 24.8514 29.2582 24.9701 29.2582C25.1283 29.2582 25.2472 29.1789 25.3261 29.1C25.4843 28.9021 25.4843 28.5855 25.2469 28.4274L23.1102 26.6466L25.2469 24.8659C25.4846 24.6677 25.4846 24.3513 25.3261 24.1534Z"
                    fill="white"
                    stroke="white"
                    stroke-width="0.322114"
                  />
                  <path
                    d="M33.9184 26.3738L31.3464 24.1972C31.1485 24.039 30.8319 24.0785 30.6737 24.2765C30.5155 24.4744 30.5155 24.791 30.753 24.9491L32.8896 26.7299L30.753 28.5106C30.555 28.6688 30.5155 28.9854 30.6737 29.1833C30.753 29.3021 30.9111 29.3415 31.0298 29.3415C31.1485 29.3415 31.2672 29.3021 31.3465 29.2228L33.9184 27.0462C34.0372 26.967 34.0766 26.8088 34.0766 26.6901C34.0768 26.5717 34.0372 26.4924 33.9184 26.3738Z"
                    fill="white"
                    stroke="white"
                    stroke-width="0.322114"
                  />
                  <path
                    d="M29.3304 21.9005C29.093 21.8213 28.8159 21.9797 28.737 22.2171L26.0065 30.9627C25.9272 31.2001 26.0856 31.4772 26.3231 31.5562C26.3625 31.5562 26.4022 31.5957 26.4811 31.5957C26.6791 31.5957 26.877 31.477 26.9559 31.2395L29.6467 22.5335C29.7259 22.2563 29.6073 21.9798 29.3304 21.9005Z"
                    fill="white"
                    stroke="white"
                    stroke-width="0.322114"
                  />
                  <path
                    d="M35.8948 9H20.1057C18.879 9 17.8105 10.0288 17.8105 11.2952V43.7055C17.8105 44.9322 18.8394 46.0007 20.1057 46.0007H35.8948C37.1213 46.0007 38.1898 44.9719 38.1898 43.7055L38.1896 11.2952C38.1896 10.0685 37.1214 9 35.8948 9ZM36.9237 43.7055C36.9237 44.2989 36.4884 44.7344 35.8949 44.7344H20.106C19.5125 44.7344 19.0771 44.299 19.0771 43.7055V40.1835H36.9634L36.9631 43.7055H36.9237ZM36.9237 38.9565H19.0375V13.5903H36.9237V38.9565ZM36.9237 12.3239H19.0375V11.2951C19.0375 10.7016 19.4727 10.2663 20.0662 10.2663H25.7248C25.7643 10.5037 25.9621 10.6621 26.1996 10.6621H29.6819C29.9193 10.6621 30.1172 10.4642 30.1566 10.2663H35.8153C36.4087 10.2663 36.8441 10.7016 36.8441 11.2951L36.8445 12.3239H36.9237Z"
                    fill="white"
                    stroke="white"
                    stroke-width="0.322114"
                  />
                  <path
                    d="M29.0488 42.4832C29.0488 43.0735 28.5704 43.5518 27.9803 43.5518C27.3905 43.5518 26.9121 43.0735 26.9121 42.4832C26.9121 41.8934 27.3905 41.415 27.9803 41.415C28.5705 41.415 29.0488 41.8934 29.0488 42.4832Z"
                    fill="white"
                    stroke="white"
                    stroke-width="0.322114"
                  />
                </svg>

                <span className="lg:text-[20px] text-[16px] font-[600]  ">
                  Internal Expertise
                </span>
                <p className="lg:text-[16px]  text-[12px]">
                  Our team boasts in-house expertise, with skilled professionals
                  who possess deep knowledge and experience in HubSpot
                  solutions. This ensures that you receive direct, hands-on
                  support from specialists who are dedicated to delivering the
                  best results for your business.
                </p>
              </div>

              <div className="lg:py-3 lg:px-5 px-3 py-2 lg:w-[400px] border  border-blue-500 hover:scale-105 rounded-md flex flex-col gap-3">
                <svg
                  width="56"
                  height="56"
                  viewBox="0 0 56 56"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <path
                    d="M37.3333 0H18.6667C8.35735 0 0 8.35735 0 18.6667V37.3333C0 47.6426 8.35735 56 18.6667 56H37.3333C47.6426 56 56 47.6426 56 37.3333V18.6667C56 8.35735 47.6426 0 37.3333 0Z"
                    fill="#4C49E1"
                  />
                  <path
                    d="M25.3261 24.1534C25.168 23.9555 24.8514 23.916 24.6535 24.0741L22.0815 26.2508C21.9628 26.3299 21.9233 26.4881 21.9233 26.6068C21.9233 26.765 22.0025 26.884 22.0815 26.9629L24.6535 29.1395C24.7326 29.2187 24.8514 29.2582 24.9701 29.2582C25.1283 29.2582 25.2472 29.1789 25.3261 29.1C25.4843 28.9021 25.4843 28.5855 25.2469 28.4274L23.1102 26.6466L25.2469 24.8659C25.4846 24.6677 25.4846 24.3513 25.3261 24.1534Z"
                    fill="white"
                    stroke="white"
                    stroke-width="0.322114"
                  />
                  <path
                    d="M33.9184 26.3738L31.3464 24.1972C31.1485 24.039 30.8319 24.0785 30.6737 24.2765C30.5155 24.4744 30.5155 24.791 30.753 24.9491L32.8896 26.7299L30.753 28.5106C30.555 28.6688 30.5155 28.9854 30.6737 29.1833C30.753 29.3021 30.9111 29.3415 31.0298 29.3415C31.1485 29.3415 31.2672 29.3021 31.3465 29.2228L33.9184 27.0462C34.0372 26.967 34.0766 26.8088 34.0766 26.6901C34.0768 26.5717 34.0372 26.4924 33.9184 26.3738Z"
                    fill="white"
                    stroke="white"
                    stroke-width="0.322114"
                  />
                  <path
                    d="M29.3304 21.9005C29.093 21.8213 28.8159 21.9797 28.737 22.2171L26.0065 30.9627C25.9272 31.2001 26.0856 31.4772 26.3231 31.5562C26.3625 31.5562 26.4022 31.5957 26.4811 31.5957C26.6791 31.5957 26.877 31.477 26.9559 31.2395L29.6467 22.5335C29.7259 22.2563 29.6073 21.9798 29.3304 21.9005Z"
                    fill="white"
                    stroke="white"
                    stroke-width="0.322114"
                  />
                  <path
                    d="M35.8948 9H20.1057C18.879 9 17.8105 10.0288 17.8105 11.2952V43.7055C17.8105 44.9322 18.8394 46.0007 20.1057 46.0007H35.8948C37.1213 46.0007 38.1898 44.9719 38.1898 43.7055L38.1896 11.2952C38.1896 10.0685 37.1214 9 35.8948 9ZM36.9237 43.7055C36.9237 44.2989 36.4884 44.7344 35.8949 44.7344H20.106C19.5125 44.7344 19.0771 44.299 19.0771 43.7055V40.1835H36.9634L36.9631 43.7055H36.9237ZM36.9237 38.9565H19.0375V13.5903H36.9237V38.9565ZM36.9237 12.3239H19.0375V11.2951C19.0375 10.7016 19.4727 10.2663 20.0662 10.2663H25.7248C25.7643 10.5037 25.9621 10.6621 26.1996 10.6621H29.6819C29.9193 10.6621 30.1172 10.4642 30.1566 10.2663H35.8153C36.4087 10.2663 36.8441 10.7016 36.8441 11.2951L36.8445 12.3239H36.9237Z"
                    fill="white"
                    stroke="white"
                    stroke-width="0.322114"
                  />
                  <path
                    d="M29.0488 42.4832C29.0488 43.0735 28.5704 43.5518 27.9803 43.5518C27.3905 43.5518 26.9121 43.0735 26.9121 42.4832C26.9121 41.8934 27.3905 41.415 27.9803 41.415C28.5705 41.415 29.0488 41.8934 29.0488 42.4832Z"
                    fill="white"
                    stroke="white"
                    stroke-width="0.322114"
                  />
                </svg>

                <span className="lg:text-[20px] text-[16px] font-[600]  ">
                  Data Security and Confidentiality
                </span>
                <p className="lg:text-[16px]  text-[12px]">
                  We prioritize the security and confidentiality of your data,
                  ensuring that all sensitive information is protected through
                  robust measures. Our commitment to maintaining privacy and
                  adhering to industry standards ensures that your business can
                  trust us with the highest level of data protection.
                </p>
              </div>

              <div className="lg:py-3 lg:px-5 px-3 py-2 lg:w-[400px] border  border-blue-500 hover:scale-105 rounded-md flex flex-col gap-3">
                <svg
                  width="56"
                  height="56"
                  viewBox="0 0 56 56"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <path
                    d="M37.3333 0H18.6667C8.35735 0 0 8.35735 0 18.6667V37.3333C0 47.6426 8.35735 56 18.6667 56H37.3333C47.6426 56 56 47.6426 56 37.3333V18.6667C56 8.35735 47.6426 0 37.3333 0Z"
                    fill="#4C49E1"
                  />
                  <path
                    d="M25.3261 24.1534C25.168 23.9555 24.8514 23.916 24.6535 24.0741L22.0815 26.2508C21.9628 26.3299 21.9233 26.4881 21.9233 26.6068C21.9233 26.765 22.0025 26.884 22.0815 26.9629L24.6535 29.1395C24.7326 29.2187 24.8514 29.2582 24.9701 29.2582C25.1283 29.2582 25.2472 29.1789 25.3261 29.1C25.4843 28.9021 25.4843 28.5855 25.2469 28.4274L23.1102 26.6466L25.2469 24.8659C25.4846 24.6677 25.4846 24.3513 25.3261 24.1534Z"
                    fill="white"
                    stroke="white"
                    stroke-width="0.322114"
                  />
                  <path
                    d="M33.9184 26.3738L31.3464 24.1972C31.1485 24.039 30.8319 24.0785 30.6737 24.2765C30.5155 24.4744 30.5155 24.791 30.753 24.9491L32.8896 26.7299L30.753 28.5106C30.555 28.6688 30.5155 28.9854 30.6737 29.1833C30.753 29.3021 30.9111 29.3415 31.0298 29.3415C31.1485 29.3415 31.2672 29.3021 31.3465 29.2228L33.9184 27.0462C34.0372 26.967 34.0766 26.8088 34.0766 26.6901C34.0768 26.5717 34.0372 26.4924 33.9184 26.3738Z"
                    fill="white"
                    stroke="white"
                    stroke-width="0.322114"
                  />
                  <path
                    d="M29.3304 21.9005C29.093 21.8213 28.8159 21.9797 28.737 22.2171L26.0065 30.9627C25.9272 31.2001 26.0856 31.4772 26.3231 31.5562C26.3625 31.5562 26.4022 31.5957 26.4811 31.5957C26.6791 31.5957 26.877 31.477 26.9559 31.2395L29.6467 22.5335C29.7259 22.2563 29.6073 21.9798 29.3304 21.9005Z"
                    fill="white"
                    stroke="white"
                    stroke-width="0.322114"
                  />
                  <path
                    d="M35.8948 9H20.1057C18.879 9 17.8105 10.0288 17.8105 11.2952V43.7055C17.8105 44.9322 18.8394 46.0007 20.1057 46.0007H35.8948C37.1213 46.0007 38.1898 44.9719 38.1898 43.7055L38.1896 11.2952C38.1896 10.0685 37.1214 9 35.8948 9ZM36.9237 43.7055C36.9237 44.2989 36.4884 44.7344 35.8949 44.7344H20.106C19.5125 44.7344 19.0771 44.299 19.0771 43.7055V40.1835H36.9634L36.9631 43.7055H36.9237ZM36.9237 38.9565H19.0375V13.5903H36.9237V38.9565ZM36.9237 12.3239H19.0375V11.2951C19.0375 10.7016 19.4727 10.2663 20.0662 10.2663H25.7248C25.7643 10.5037 25.9621 10.6621 26.1996 10.6621H29.6819C29.9193 10.6621 30.1172 10.4642 30.1566 10.2663H35.8153C36.4087 10.2663 36.8441 10.7016 36.8441 11.2951L36.8445 12.3239H36.9237Z"
                    fill="white"
                    stroke="white"
                    stroke-width="0.322114"
                  />
                  <path
                    d="M29.0488 42.4832C29.0488 43.0735 28.5704 43.5518 27.9803 43.5518C27.3905 43.5518 26.9121 43.0735 26.9121 42.4832C26.9121 41.8934 27.3905 41.415 27.9803 41.415C28.5705 41.415 29.0488 41.8934 29.0488 42.4832Z"
                    fill="white"
                    stroke="white"
                    stroke-width="0.322114"
                  />
                </svg>

                <span className="lg:text-[20px] text-[16px] font-[600]  ">
                  The Pioneers and Leaders
                </span>
                <p className="lg:text-[16px]  text-[12px]">
                  We are the pioneers and leaders in providing top-tier HubSpot
                  solutions, offering unmatched expertise and exceptional
                  service. Our reputation for excellence makes us the go-to
                  choice for businesses seeking the best in the industry.
                </p>
              </div>
            </div>
          </div>
        </div>
      </div>
      <div className="flex justify-center py-5 lg:py-10 px-3">
        <div className="lg:w-[90%] flex flex-col gap-5 items-center">
          <span className="lg:text-[28px]  text-[20px] text-center  font-[700]">
            How we can support you as a HubSpot Solution Partner.
          </span>
          <div className=" flex flex-col lg:flex-row gap-4">
            <div className=" shadow-xl flex flex-col gap-2 lg:px-6 lg:py-4 py-3 p-2 border  rounded-md lg:w-[400px]">
              <span className="lg:text-[20px] text-[16px] font-[600] text-orange-500 ">
                Boost the capabilities.
              </span>
              <p className="lg:text-[16px] text-[12px]  text-gray-700 ">
                We help boost the capabilities of your business by optimizing
                HubSpot’s tools to fit your specific needs. Our tailored
                solutions enhance your efficiency, streamline processes, and
                drive better results across marketing, sales, and customer
                service.
              </p>
            </div>
            <div className=" shadow-xl flex flex-col gap-2 lg:px-6 lg:py-4 py-3 p-2 border  rounded-md lg:w-[400px]">
              <span className="lg:text-[20px] text-[16px] font-[600] text-orange-500 ">
                Instant Support
              </span>
              <p className="lg:text-[16px] text-[12px]  text-gray-700 ">
                We provide instant support to address any issues or questions
                you may have, ensuring that you receive timely assistance
                whenever you need it. Our team is always ready to offer quick
                solutions, keeping your business running smoothly.
              </p>
            </div>

            <div className=" shadow-xl flex flex-col gap-2 lg:px-6 lg:py-4 py-3 p-2 border  rounded-md lg:w-[400px]">
              <span className="lg:text-[20px] text-[16px] font-[600] text-orange-500 ">
                Analytical Skill Mindset
              </span>
              <p className="lg:text-[16px] text-[12px]  text-gray-700 ">
                We foster an analytical mindset that focuses on data-driven
                decision-making and problem-solving. By leveraging insights and
                evaluating key metrics, we help you make informed choices that
                drive business growth and improve performance.
              </p>
            </div>
          </div>
        </div>
      </div>

     
    </div>
  );
};

export default Hubspotsection2;
